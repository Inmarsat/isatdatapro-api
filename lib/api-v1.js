...
/**
 * Allows overriding the default API remapping with a string argument 'preserveNativeApi'
 * @private
 * @param {Object[]} args An array of undocumented arguments including a string
 */
 ...
 * @returns {number|string} see {@link TerminalWakeupPeriods}
 */
function getWakeupPeriod(seconds, asString) {
  const wakeupValue = TerminalWakeupPeriods[seconds];
  if (asString) {
    return getKeyByValue_(modemWakeupPeriods, wakeupValue);
  }
  return wakeupValue;
}
 ...
 * @property {number} mobileWakeupPeriod An enumerated value corresponding to the modem's configurable wakeupPeriod
 * @property {number} mobileSleepSeconds If the modem is configured for low power mode, the number of seconds of sleep duration
...
async function submitForwardMessages(auth, messages) {
  ...
  remappedResponse.submissions.forEach(submission => {
    submission.mobileWakeupPeriod = getWakeupPeriod(submission.mobileSleepSeconds);
  });
