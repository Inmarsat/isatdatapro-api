...
function remapJsonKeys_(jsonObject) {
  ...
  function camelize(str) {
    ...
    const specialCases = {
      ...
      //"SIN": 'ServiceIdentifier',
      //"MIN": 'MessageIdentifier',
      ...
      "CreateUTC": 'CreateUtc',
      "ForwardMessageID": 'ForwardMessageId',  // MessageId ?
      "LastRegistrationUTC": 'LastRegistrationUtc',
    }
 ...
 * @property {string} NextStartUTC Timestamp 'YYYY-MM-DD hh:mm:ss' for the next high water mark retrieval, empty string if none
...
async function getMobileOriginatedMessages(auth, filter) {
  ...
  if (typeof (filter.startMessageId) === 'number') {
  ...
  } else if (filter.startTimeUtc instanceof Date) {
    if (filter.startTimeUtc instanceof Date) {
      ...
      throw new Error(`Invalid startTimeUtc ${apiFilter.startTimeUtc}`);
    }
  } else {
  ...
  if (filter.endTimeUtc) {
    if (filter.endTimeUtc instanceof Date) {
      ...
      throw new Error(`Invalid endTimeUtc ${apiFilter.endTimeUtc}`);
    }
  }
...
async function getMobileTerminatedStatuses(auth, filter) {
  ...
  if (filter.startTimeUtc) {
    if (filter.startTimeUtc instanceof Date) {
      apiFilter.start_utc = dateToIdpTime(filter.startTimeUtc);
    } else if (!validateIdpTimeFormat_(apiFilter.startTimeUtc)) {
      throw new Error(`Invalid startTimeUtc ${apiFilter.startTimeUtc}`);
    }
  }
  if (!filter.ids && !filter.startTimeUtc) {
    throw new Error('Missing filter ids or startTimeUtc');
  }
  if (filter.endTimeUtc) {
    if (filter.endTimeUtc instanceof Date) {
      apiFilter.end_utc = dateToIdpTime(filter.endTimeUtc);
    } else if (!validateIdpTimeFormat_(apiFilter.endTimeUtc)) {
      throw new Error(`Invalid endTimeUtc ${apiFilter.endTimeUtc}`);
    }
  }
