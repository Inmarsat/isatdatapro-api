...
const apiUrl = config.get('gatewayUrl')
// const camelCase = config.get('camelCase') // if false preserves native API case
...
async function getErrorDefinitions() {
  ...
  //if (camelCase) errorDefinitions = camelizeJsonKeys(errorDefinitions);
...
async function getMobileOriginatedMessages(auth, filter) {
  ...
  //if (camelCase) response = camelizeJsonKeys(response);
  return response;
}
