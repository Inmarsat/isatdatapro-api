...
let apiUrl = config.get('idpGatewayUrl');
...
function idpTimeToDate(idpTime) {
  if (validateIdpTimeFormat_(idpTime)) {
    let isoTime = idpTime.replace(' ', 'T') + 'Z';
  ...
  function camelizeObjectKeys(obj) {
    ...
    for (let key in obj) {
      ...
      if (obj[key] instanceof Array && !key.includes('Raw')) {
        ...
        if (false && key.includes('UTC') && obj[key] !== "") {
 ...
 * @property {number} serviceCode Service Identification Number the first byte of payload optionally used for codec
 * @property {number} messageCode Message Identification Number the second byte of payload optionally used for codec
 ...
 * @property {string} receiveTime A timestamp YYYY-MM-DD HH:MM:SS when the message was received at the Satellite Earth Station
 ...
 * @property {string} mailboxTime A timestamp YYYY-MM-DD HH:MM:SS when the message is ready for retrieval in the Mailbox
 ...
 * @property {string|Date} [startTimeUtc] Start time (high water mark from prior NextStartUTC) timestamp format 'YYYY-MM-DD hh:mm:ss'
 ...
 * @property {string} nextStartTime Timestamp 'YYYY-MM-DD hh:mm:ss' for the next high water mark retrieval, empty string if none
...
async function getReturnMessages(auth, filter) {
  ...
  if (typeof (filter.startMessageId) === 'number') {
    ...
    if (filter.startTimeUtc instanceof Date) {
      apiFilter.start_utc = dateToIdpTime(filter.startTimeUtc);
    } else if (validateIdpTimeFormat_(filter.startTimeUtc)) {
  ...
  if (filter.endTimeUtc) {
    if (filter.endTimeUtc instanceof Date) {
      apiFilter.end_utc = dateToIdpTime(filter.endTimeUtc);
    } else if (!validateIdpTimeFormat_(filter.endTimeUtc)) {
 ...
 * @property {string} stateTime The timestamp 'YYYY-MM-DD hh:mm:ss' of the last reported state of the message
 * @property {number} mobileWakeupPeriod A [something] indicating if the modem is configured for low power sleep
 * @property {string} scheduledSendTime A timestamp 'YYYY-MM-DD hh:mm:ss' for the scheduled message delivery to a low power modem
 */
 ...
 * @property {string} mailboxTime A timestamp assigned by the MGS upon submission (get_forward_messages only)
 * @property {string} stateTime A timestamp of the most recent State (get_forward_messages only)
 ...
 * @property {string} stateTime The timestamp 'YYYY-MM-DD hh:mm:ss' of the reported state of the message
 ...
 * @property {(string|null)} nextStartTime The timestamp 'YYYY-MM-DD hh:mm:ss' for the next retrieval, null if More is false
 ...
 * @property {string} lastRegistrationTime A timestamp format 'YYYY-MM-DD hh:mm:ss' of the last Registration message
